...
!!----------------------------------------------------------------------------
   ...
   SUBROUTINE usr_def_ist_2d( vgr, kkk, pvol, pt_v)
      ...
      REAL(wp), DIMENSION(A2D(nn_hls)) :: z_dz_t, z_dz_u, z_dz_v, z_dz_f ! e3 at the T, U, V and F 
                                                                         ! points in the cell  
      REAL(wp), DIMENSION(A2D(nn_hls)) :: zt_t,   zt_u,   zt_v,   zt_f   ! e3*pt value at the 
      ...
      DO_2D (0, 0, 0, 0)      !   need to calculate for lower halo pts  
         ...
         IF ( ln_init_curved ) THEN  
            ...
            z_dz_t(ji,jj) = ocean_depth( glamt(ji,jj), gphit(ji,jj) ) / REAL ( jpkm1 )
            z_dz_u(ji,jj) = ocean_depth( glamu(ji,jj), gphiu(ji,jj) ) / REAL ( jpkm1 )
            z_dz_v(ji,jj) = ocean_depth( glamv(ji,jj), gphiv(ji,jj) ) / REAL ( jpkm1 )
            z_dz_f(ji,jj) = ocean_depth( glamf(ji,jj), gphif(ji,jj) ) / REAL ( jpkm1 )
         ELSE    
            ...
            SELECT CASE (vgr)
               CASE ( "T" )
                  ...
                  z_dz_t(ji,jj) = e3t_0(ji,jj,kkk)
                  z_dz_u(ji,jj) = e3u_0(ji,jj,kkk)
                  z_dz_v(ji,jj) = e3v_0(ji,jj,kkk)
                  z_dz_f(ji,jj) = e3f_0(ji,jj,kkk)
               CASE ( "W" )
                  ...
                  z_dz_t(ji,jj) = e3w_0 (ji,jj,kkk)
                  z_dz_u(ji,jj) = e3uw_0(ji,jj,kkk)
                  z_dz_v(ji,jj) = e3vw_0(ji,jj,kkk)
                  z_dz_f(ji,jj) = 0.25 * ( e3w_0(ji,jj  ,kkk) + e3w_0(ji+1,jj  ,kkk) &
