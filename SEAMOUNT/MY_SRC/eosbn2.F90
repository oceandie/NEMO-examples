...
CONTAINS
   ...
   SUBROUTINE eos_insitu_t( pts, ktts, prd, ktrd, pdep, ktdep )
      ...
      CASE( np_eeos )                !==  exponential SEAMOUNT EOS  ==!
         !
         DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, jpkm1 )
            zt  = pts  (ji,jj,jk,jp_tem) - ( 28._wp + (rn_drho/rn_a0)*EXP( -pdep(ji,jj,jk)/1000._wp ) ) 
            ztm = tmask(ji,jj,jk)
            !
            zn = - rn_a0 * zt
            !                                 
   ...
   SUBROUTINE eos_insitu_pot_t( pts, ktts, prd, ktrd, prhop, ktrhop, pdep, ktdep )
      ...
      ll_dbg_eos = .FALSE.  ! temporary write statement - should be in a namelist
      ...
      CASE( np_eeos )                !==  exponential SEAMOUNT EOS  ==!
         !
         DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, jpkm1 )
            z_tbg(ji,jj,jk) = 28._wp + (rn_drho/rn_a0)*EXP( -pdep(ji,jj,jk)/1000._wp )
            zt  = pts  (ji,jj,jk,jp_tem) - z_tbg(ji,jj,jk) 
            ztm = tmask(ji,jj,jk)
            !
            zn = - rn_a0 * zt
            !                                                     ! potential density referenced at the surface
         ...
         !
         IF (ll_dbg_eos) THEN 
            CALL dbg_eos_3dr('eos_insitu_pot_t: z_tbg ', z_tbg ) 
            CALL dbg_eos_3dr('eos_insitu_pot_t: pdep ', pdep ) 
            CALL dbg_eos_3dr('eos_insitu_pot_t: prd ', prd ) 
	 END IF 
      END SELECT
      !
   ...
   SUBROUTINE eos_insitu_2d_t( pts, ktts, pdep, ktdep, prd, ktrd )
      ...
      CASE( np_eeos )                !==  exponential SEAMOUNT EOS  ==!
         !
         DO_2D( nn_hls, nn_hls, nn_hls, nn_hls )
            zt  = pts  (ji,jj,jp_tem) - ( 28._wp + (rn_drho/rn_a0)*EXP( -pdep(ji,jj)/1000._wp ) )
            !
            zn = - rn_a0 * zt
            !
   ...
   SUBROUTINE eos_insitu_pot_2d_t( pts, ktts, prhop, ktrhop )
      ...
      CASE( np_eeos )                !==  exponential SEAMOUNT EOS  ==!
         !
         DO_2D( nn_hls, nn_hls, nn_hls, nn_hls )
            zt  = pts  (ji,jj,jp_tem) - ( 28._wp + (rn_drho/rn_a0) )
            ztm = tmask(ji,jj,1)
            !
            zn = - rn_a0 * zt
            !
   ...
   SUBROUTINE rab_3d_t( pts, ktts, pab, ktab, Kmm )
      ...
      CASE( np_eeos )                !==  exponential SEAMOUNT EOS  ==!
         !
         DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, jpkm1 )
            zt  = pts  (ji,jj,jk,jp_tem) - ( 28._wp + (rn_drho/rn_a0)*EXP( -gdept(ji,jj,jk,Kmm)/1000._wp ) )
            ztm = tmask(ji,jj,jk)
            !
            zn = rn_a0
            !                                 
   ...
   SUBROUTINE rab_0d( pts, pdep, pab, Kmm )
      ...
      CASE( np_eeos )                !==  exponential SEAMOUNT EOS  ==!
         !
         zt  = pts  (jp_tem) - ( 28._wp + (rn_drho/rn_a0)*EXP( -pdep / 1000._wp ) )	 
         zn =   rn_a0
         !                                 
         pab(jp_tem) = zn * r1_rho0    ! alpha
         pab(jp_sal) = zn * r1_rho0    ! beta
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nameos in configuration namelist' )
   ...
   SUBROUTINE dbg_eos_3dr( cc_array, pr_3d )
   ...
   ln_dbg_ij = .true. ;  ln_dbg_ik = .true. ;  ln_dbg_jk = .true. 
   ki_dbg_min = 40;   ki_dbg_max = 43;    kj_dbg_min = 30
   kj_dbg_max = 34;   kk_dbg_min =  1;    kk_dbg_max = 11  
   ki_dbg_cen = 42;   kj_dbg_cen = 32;    kk_dbg_cen =  5
